...
When we are in R and want to set the value of the gauge to 75, we must make sure the final data the chart gets looks like this. In this case the data representation we're after is simple. However, in general this often is harder.
...
Within the "inst" folder an "htmlwidget" subfolder is created. Here you'll find the following files:
  ...
  -C3Gauge.yaml: This file lists all dependencies and their location in the folder structure for the widget. Because we only used bower to get our external dependency this file is already properly filled.
...
In the default code the message argument is any data that is passed on to the renderValue function in the C3Gauge.js code. The width and height information is passed on to the resize function in the same file. For now we will focus on the data part and deal with the re-sizing later.
...
Going back to our javascript code we have to make one modification: in stead of passing our static gaugeData to the C3 generate function we pass on our list created in R. We change the line where our data is specified to the following.
