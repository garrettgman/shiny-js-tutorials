...
c3 has three [different ways of providing components with data](http://c3js.org/examples.html#data): row based, column based or as a JSON. In our examples we typically pick the JSON format as shiny uses jsonlite to convert R data structures to JSON (see above). in addition to passing the data we also tell c3.js that we want the chart to be of type 'gauge'.
